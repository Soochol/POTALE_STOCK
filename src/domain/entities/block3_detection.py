...
class Block3Detection:
    ...
    # 기본 정보 (기본값 있는 필드)
    block3_id: str = ""  # 블록3 고유 ID (UUID, __post_init__에서 생성)
    ...
    # 직전 블록2 정보
    prev_block2_id: Optional[int] = None  # 참조한 블록2의 ID
    prev_block2_peak_price: Optional[float] = None  # 블록2 최고가
    prev_block2_peak_volume: Optional[int] = None  # 블록2 최고 거래량

    # 블록3 최고가 추적
    peak_price: Optional[float] = None  # 블록3 기간 중 최고가
    peak_date: Optional[date] = None  # 최고가 달성일
    peak_gain_ratio: Optional[float] = None  # 최고가 상승률 (%)
    peak_volume: Optional[int] = None  # 블록3 기간 중 최고 거래량
