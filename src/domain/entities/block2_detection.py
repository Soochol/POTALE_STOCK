...
class Block2Detection:
    ...
    # 기본 정보 (기본값 있는 필드)
    block2_id: str = ""  # 블록2 고유 ID (UUID, __post_init__에서 생성)
    ...
    # 직전 블록1 정보
    prev_block1_id: Optional[int] = None  # 참조한 블록1의 ID
    prev_block1_peak_price: Optional[float] = None  # 블록1 최고가
    prev_block1_peak_volume: Optional[int] = None  # 블록1 최고 거래량

    # 블록2 최고가 추적
    peak_price: Optional[float] = None  # 블록2 기간 중 최고가
    peak_date: Optional[date] = None  # 최고가 달성일
    peak_gain_ratio: Optional[float] = None  # 최고가 상승률 (%)
    peak_volume: Optional[int] = None  # 블록2 기간 중 최고 거래량

    # 종료 정보
    duration_days: Optional[int] = None  # 지속 기간
    exit_reason: Optional[str] = None  # 종료 사유 (ma_break, three_line_reversal, body_middle, block3_started)
