"""
Block5 Checker Service - 블록5 진입/종료 조건 검사 서비스
"""
...
class Block5Checker:
    ...
    def check_entry(
        ...
        Args:
            ...
            prev_seed_block4: Block4 Seed (Block5 조건의 volume_ratio, low_price_margin 비교 기준)

        Returns:
            모든 조건 만족 여부
        """
        # 1. Block1+Block2+Block3+Block4 기본 조건 검사
        ...
        # 2. 블록5 추가 조건 검사
        if condition.block5_volume_ratio is not None:
            if prev_seed_block4 is not None and prev_seed_block4.peak_volume is not None:
                ratio = condition.block5_volume_ratio / 100.0
                required_volume = prev_seed_block4.peak_volume * ratio
                if stock.volume < required_volume:
                    return False

        if condition.block5_low_price_margin is not None:
            if prev_seed_block4 is not None and prev_seed_block4.peak_price is not None:
    ...
    def create_detection(
        ...
        condition_name: str,
        stock: Stock,
        prev_block4: Optional[Block4Detection] = None
    ) -> Block5Detection:
        """블록5 탐지 결과 생성"""
        ...
        if prev_block4:
            detection.prev_block4_id = prev_block4.id
            detection.prev_block4_peak_price = prev_block4.peak_price
            detection.prev_block4_peak_volume = prev_block4.peak_volume
    ...
    def check_cooldown(
        ...
        ticker: str,
        current_date: date,
        existing_detections: List[Block5Detection],
    ...
    def check_min_candles(
        ...
        if prev_block4 is None:
            return True

        candles_count = self._count_candles_between(
            prev_block4.started_at,
            current_date,
            all_stocks
        )
    ...
    def check_max_candles(
        ...
        """블록4 시작 후 최대 캔들 수 확인"""
        if prev_block4 is None or max_candles is None:
            return True
    ...
    def check_lookback_window(
        self,
        current_date: date,
        prev_seed_block4: Optional[Block4Detection],
        lookback_min_candles: Optional[int],
        lookback_max_candles: Optional[int],
        all_stocks: List[Stock],
    ) -> bool:
        """
        Lookback 검증: Block5 후보일 기준 과거에 Block4가 적절한 범위 내에 존재하는지 확인

        Args:
            current_date: Block5 후보일
            prev_seed_block4: 이전 Seed Block4 탐지 결과
            lookback_min_candles: 과거 최소 캔들 범위 (None=체크 안함)
            lookback_max_candles: 과거 최대 캔들 범위 (None=체크 안함)
            all_stocks: 전체 주식 데이터 (날짜순 정렬)

        Returns:
            조건 만족 여부 (True: Block4가 적절한 범위에 있음, False: 범위 밖)
        """
        # 조건이 없으면 스킵
        ...
        # 이전 Block4가 없으면 스킵
        if prev_seed_block4 is None:
            return True

        # 후보일에서 Block4 시작일까지의 캔들 수 계산
        candles_count = self._count_candles_between(
            prev_seed_block4.started_at,
            current_date,
            all_stocks
