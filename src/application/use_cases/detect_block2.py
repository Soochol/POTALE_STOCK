"""
Detect Block2 Use Case - 블록2 탐지 유스케이스
"""
...
from ..services.block2_checker import Block2Checker
from ...infrastructure.repositories.block2_repository import Block2Repository
...
class DetectBlock2UseCase:
    ...
    주요 기능:
    1. 블록2 진입 조건 검사 (블록1 조건 + 블록2 추가 조건)
    2. 블록2 종료 조건 검사 (기존 활성 블록2)
    3. 직전 블록1 찾기 (날짜 기준)
    4. 블록2 탐지 결과 저장
    """
    ...
    def __init__(
        ...
        checker: Optional[Block2Checker] = None
    ):
        ...
        self.checker = checker or Block2Checker()
    ...
    def execute(
        ...
        if not condition.validate():
            raise ValueError(f"Invalid Block2Condition: {condition}")
    ...
    def _detect_ticker(
        ...
        """
        # 지표 계산 (블록1 조건 사용)
        block1_condition = condition.block1_condition
        ...
        # 기존 블록2 조회
        existing_block2s = self.block2_repository.find_by_ticker(ticker)
        ...
        for i, stock in enumerate(stocks_with_indicators):
            ...
            # 1. 기존 활성 블록2의 최고가/거래량 갱신
            self._update_peaks(existing_block2s, stock)

            # 2. 기존 활성 블록2의 종료 조건 검사
            ...
            # 3. 새로운 블록2 진입 조건 검사
            # 직전 블록1 찾기 (현재 날짜 이전에 완료된 가장 최근 블록1)
            ...
            if self._check_entry_conditions(
                ...
                # 저장
                saved_detection = self.block2_repository.save(detection)
                new_detections.append(saved_detection)

                # 기존 리스트에 추가 (종료 조건 검사용)
                existing_block2s.append(saved_detection)
    ...
    def _check_entry_conditions(
        ...
        existing_block2s: List[Block2Detection],
        all_stocks: List[Stock]
    ) -> bool:
        """블록2 진입 조건 검사"""
        # 1. 블록2 진입 조건 검사 (블록1 조건 + 블록2 추가 조건)
        if not self.checker.check_entry(condition, stock, prev_stock, prev_block1):
            return False
    ...
    def _update_peaks(
        ...
        """
        기존 활성 블록2의 최고가/거래량 갱신

        Args:
            existing_block2s: 기존 블록2 리스트
            current_stock: 현재 주식 데이터
        """
        for detection in existing_block2s:
            # 활성 블록2만 검사
            ...
            if (detection.peak_price != old_peak_price or
                ...
                self.block2_repository.update_peak(
                    detection.block2_id,
    ...
    def _check_exit_conditions(
        ...
        for detection in existing_block2s:
            ...
            if exit_reason:
                ...
                # DB 업데이트
                self.block2_repository.update_status(
                    detection.block2_id,
                    status="completed",
    ...
    def get_active_blocks(self, ticker: str) -> List[Block2Detection]:
        """
        종목의 활성 블록2 조회
        ...
        Returns:
            활성 블록2 리스트
        """
        return self.block2_repository.find_active_by_ticker(ticker)
    ...
    def get_all_blocks(
        ...
        Returns:
            블록2 리스트
        """
        return self.block2_repository.find_by_ticker(
            ticker,
            status=None,
            from_date=from_date,
            to_date=to_date
